...
const assertWebViewTagEnabled = function (event) {
  if (!isWebViewTagEnabled(event.sender)) {
    throw new Error('<webview> disabled')
  }
}

ipcMainUtils.handle('ELECTRON_GUEST_VIEW_MANAGER_CREATE_GUEST', function (event, params) {
  assertWebViewTagEnabled(event)
  return createGuest(event.sender, params)
})

ipcMainUtils.handle('ELECTRON_GUEST_VIEW_MANAGER_CREATE_GUEST_SYNC', function (event, params) {
  assertWebViewTagEnabled(event)
  return createGuest(event.sender, params)
})

ipcMainInternal.on('ELECTRON_GUEST_VIEW_MANAGER_DESTROY_GUEST', function (event, guestInstanceId) {
  try {
    assertWebViewTagEnabled(event)
...
ipcMainInternal.on('ELECTRON_GUEST_VIEW_MANAGER_ATTACH_GUEST', function (event, embedderFrameId, elementInstanceId, guestInstanceId, params) {
  try {
    assertWebViewTagEnabled(event)
...
ipcMainUtils.handle('ELECTRON_GUEST_VIEW_MANAGER_ASYNC_CALL', function (event, guestInstanceId, method, args) {
  assertWebViewTagEnabled(event)
...
ipcMainUtils.handle('ELECTRON_GUEST_VIEW_MANAGER_SYNC_CALL', function (event, guestInstanceId, method, args) {
  assertWebViewTagEnabled(event)
